...
func main() {
	...
	threshold := 30*1e6 - (70*1e6 - rootDir.size)
	fmt.Println("threshold:\t", threshold)

	fmt.Println("smallest:\t\t", rootDir.smallestDirBiggerThanThreshold(threshold))
	fmt.Println("sum:\t\t", rootDir.sumLess100000())
}

func (d *Directory) smallestDirBiggerThanThreshold(threshold int) (sum int) {
	var size int = 1e10
	for _, dir := range d.subdirs {
		ret := dir.smallestDirBiggerThanThreshold(threshold)
		if ret < size {
			size = ret
		}
	}
	if d.size >= threshold && d.size < size {
		size = d.size
	}
